...
if __name__ == "__main__":
    ...
    if config['model_name'] == 'marl' and config['mode'] == 'train':
     ...
     ########################################################################
        ...
        noise_levels_to_test = [0.0, 0.1, 0.3, 0.5, 0.7, 1.0]
        ...
        for noise_std in noise_levels_to_test:
            ...
            # c) 初始化当前噪声水平下的测试统计变量
            test_episodes = config.get('testing_episodes', 200)  # 为加快分析，可以减少测试回合数
            ...
            avg_se = total_test_se / total_steps
            avg_reward = total_test_reward / total_steps
            avg_qos_violations = total_qos_violations / total_steps # QoS违规数

            # f) 记录结果
            sensitivity_results[noise_std] = {'avg_se': avg_se,
                                              'avg_reward': avg_reward,
                                              'avg_qos_violations': avg_qos_violations
                                                }
        ...
        print("========================================================")
        print(f"{'Noise Std (预测噪声标准差)':<28} | {'Avg. Spectrum Efficiency (平均频谱效率)':<40}")
        print("-" * 70)
        for noise_std, results in sensitivity_results.items():
            print(f"{noise_std:<28.2f} | {results['avg_se']:<40.4f} | {results['avg_qos_violations']:<30.4f}")
        print("========================================================")
