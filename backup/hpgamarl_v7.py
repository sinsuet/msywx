    ...
    def select_action(self, base_obs, power_min, power_max, use_exploration=True):
        ...
        with torch.no_grad():
            ...
            final_obs_for_power = torch.cat([
                torch.FloatTensor(base_obs).unsqueeze(0).to(self.device),
                torch.FloatTensor(np.array(channel_actions)).unsqueeze(0).to(self.device)
            ], dim=1)
    ...
    def train_step(self):
        ...
        for gnb_idx, agent in enumerate(self.agents):
            ...
            for user_idx in range(agent.num_users):
                # Actor进行一步前向传播
                # 输入：当前步的观测（在这里简化为使用固定的初始观测）和上一步的隐藏状态
                # 输出：当前步的logits和新的隐藏状态
                logits, hidden_state = agent.actor(local_obs_tensor, hidden_state)
