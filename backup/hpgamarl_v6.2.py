...
class SatTerrestrialHPGAMARLFramework:
    ...
    def train_step(self):
        ...
        global_obs_tensor = torch.FloatTensor(np.array(all_current_obs)).to(self.device)
        next_global_obs_tensor = torch.FloatTensor(np.array(all_next_obs)).to(self.device)
