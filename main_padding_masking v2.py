...
if __name__ == "__main__":
    ...
    "model_params": {
        "marl":{
            ...
            'critic_lr': 1e-4,
            'actor_lr': 3e-4,
            'warmup_episodes': 20,  # 【新增】随机探索预热的回合数
    ...
    if config['model_name'] == 'marl' and config['mode'] == 'train':
        ...
        # 【新增】从config中获取预热参数
        model_params = config['model_params']['marl']
        warmup_episodes = model_params.get('warmup_episodes', 0)

        print(f"\n--- 开始 P-MADDPG 算法训练 (预热回合数: {warmup_episodes}) ---")
            ...
            for step in range(config['steps_per_episode']):
                ...
                if episode < warmup_episodes:
                    ...
                    for gnb_idx in range(framework.env.num_gnbs):
                        actions[gnb_idx] = np.random.randint(0, framework.env.num_channels, framework.env.users_per_gnb)
