...
if __name__ == "__main__":
    ...
    config = {
        ...
        'model_name':'marl',  # marl,gdfp,randompolicy,dqn,IBR
    ...
    "model_params": {
        "marl":{
            ...
            'critic_lr': 1e-4,
            'actor_lr': 3e-4,
            'warmup_episodes': 30,  # 【新增】随机探索预热的回合数
    ...
    if config['model_name'] == 'marl' and config['mode'] == 'train':
        ...
        # 预热
        warmup_episodes = model_params.get('warmup_episodes', 20) #
        # 创建一个三维数组来存储每个gNB下每个用户的固定信道偏好
        # shape: (num_gnbs, users_per_gnb, num_channels)
        warmup_channel_preferences = np.random.rand(
            framework.env.num_gnbs,
            framework.env.users_per_gnb,
            framework.env.num_channels
        )
        # 使用softmax将随机值转换为概率分布
        e_x = np.exp(warmup_channel_preferences - np.max(warmup_channel_preferences, axis=-1, keepdims=True))
        warmup_channel_probs = e_x / e_x.sum(axis=-1, keepdims=True)


        framework = SatTerrestrialHPGAMARLFramework(config)

        print("\n--- 开始marl训练 ---")
        training_episodes = config['training_episode']
        ...
        for episode in range(training_episodes):
            ...
            for step in range(config['steps_per_episode']):
                ...
                if episode < warmup_episodes:
                    ...
                    actions = {}
                    powers = {}
                    for gnb_idx in range(framework.env.num_gnbs):
                        actions[gnb_idx] = np.random.randint(3, framework.env.num_channels, framework.env.users_per_gnb)
                        powers[gnb_idx] = np.random.uniform(framework.env.power_min, framework.env.power_max,
                                                            framework.env.users_per_gnb)
                else:
                    # 预热结束后：执行正常的、带探索的策略决策
                    actions, powers = framework.hierarchical_decision(state, use_exploration=True)
                # ==========================================================
    ...
    if config['model_name'] == 'IBR':
        ...
        MODEL_PATH = save_dir + f"best_model_{config['model_name']}.pth"
        REWARD_PATH = save_dir + f"reward_history_{config['model_name']}.csv"
        ...
        for episode in range(config['training_episode']):
            ...
            for step in range(config['steps_per_episode']):
                ...
                # 【修改】直接将完整的 state 字典传递给 select_actions
        ...
        for episode in range(config['testing_episode']):
            ...
            for step in range(config['steps_per_episode']):
                local_obs_list = [
                    qmix_framework.env.get_local_observation(g, state, config['max_gnbs'], config['max_users_per_gnb'])
                    for g in range(qmix_framework.env.num_gnbs)]

                # 关闭探索进行决策
                actions, powers, _, next_hidden_states = qmix_framework.select_actions(local_obs_list, hidden_states,
